...
- TagGroup/Tag create/update/destroy are deliberately unimplemented (tagGroups.go) - a live probe (2026-09-18) got `403 Forbidden` ("cannot create a TagGroup") from a normal signed-in session on `POST /services/v2/tag_groups`, so neither the real request body shape nor the PCO permission tier required is confirmed. Don't guess this from JSON:API convention the way `AssignSongTags` above already proved wrong for a different tag endpoint - re-verify live from a higher-permission PCO session before adding writes.
